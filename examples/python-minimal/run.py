...
XX01-XX99 are the unit of execution and are run directly. The optional
XX00_run_all.py in each phase folder is a standalone convenience runner for
that one phase; it is skipped here so nothing runs twice.
...
def discover(phase: str) -> list[Path]:
    ...
    scripts = [p for p in d.iterdir() if p.is_file()
               ...
               and not p.name.startswith(f"{phase}00_")]
