...
```
Project Inputs/ (read-only, Druck standard)
...
## Integration with Druck Standards

- Anu Architecture lives in `Technical/AnuArchitecture/` (inside the Druck-compliant project structure)
...
See `Council/Druck/docs/GIT_PROTOCOL.md` for the full standard.

---

## Error Handling

- **Project not Druck-compliant** (missing Inputs/Technical/Outputs): Warn user, suggest running `/project-initialization` first
...
*Maintained by: Druck (Performance Monitoring)*
