...
```
Project Inputs/ (read-only source data)
...
outputs/deliverables/ (tables, figures, reports)
    | manual promote
Project Outputs/YYYY.MM.DD Description/ (dated-folder convention)
```
...
2. Manual copy to `Outputs/YYYY.MM.DD Description/` (dated-folder convention)
...
- Anu Architecture lives in your project's `Technical/AnuArchitecture/` folder
- Source data read from `Inputs/` (read-only, never modified)
- Published outputs promoted to `Outputs/` (dated-folder convention, e.g. `Outputs/YYYY.MM.DD Description/`)
...
3. **MANUAL EXTRACTION from PDFs/scans** — Last resort, for methodology text or historical data only available in the book PDF.
...
Original Data: [URL of the original published dataset or appendix, if applicable]
```
...
The shipped `.gitignore` template encodes this standard; keep real keys and
regenerable cache out of version control.

---

## Error Handling

- **Project structure incomplete** (missing Inputs/Technical/Outputs): Warn user, suggest initializing the standard project folders first
